...
export function OfferImportDialog({
  ...
  async function addImportedFromDraft() {
    ...
    try {
      ...
      setMessage({
        type: "ok",
        text: `${items.length} oferta${plural} adicionada${plural} à página.`,
      });
    } catch (error) {
  ...
  async function applyCommand() {
    ...
    try {
      ...
      const nextOffers: Offer[] = source.map((value) => {
        const item = asRecord(value);
        ...
        return {
          ...
          brand:
            typeof item?.brand === "string"
              ? item.brand
              : typeof fallback.brand === "string"
                ? fallback.brand
                : "",
          size:
            typeof item?.size === "string"
              ? item.size
              : typeof fallback.size === "string"
                ? fallback.size
                : "",
    ...
    try {
      ...
      recorder.onstop = async () => {
        ...
        try {
          ...
          if (!text) {
            throw new Error(
              "Transcrevi o áudio, mas não consegui ler as ofertas. Tente falar mais devagar.",
            );
          }
          setDraft(text);
          if (modeAtStart === "comando") {
            ...
            setMessage({
              type: "ok",
              text: "Comando transcrito. Revise e clique em Aplicar correção.",
            });
          } else {
            setActiveTab("texto");
            setMessage({
              type: "ok",
              text: "Áudio transcrito. Revise o texto antes de adicionar as ofertas.",
            });
          }
        } catch (error) {
          setMessage(messageFor(error));
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            Digite, cole ou fale as ofertas. A IA identifica os produtos e preços e adiciona à
            página atual do encarte.
          </DialogDescription>
        </DialogHeader>
        ...
        >
          ...
          <TabsContent value="texto" className="mt-2 text-sm text-muted-foreground">
            Cole aqui uma lista digitada, como: arroz 5kg 29,90, feijão 1kg 7,49, óleo de soja 900ml
            6,99.
          </TabsContent>
          <TabsContent value="voz" className="mt-2 text-sm text-muted-foreground">
            Toque em Gravar áudio e fale as ofertas. O áudio será transcrito abaixo para você
            revisar antes de adicionar.
          </TabsContent>
          <TabsContent value="comando" className="mt-2 text-sm text-muted-foreground">
            Descreva uma correção para as ofertas desta página, como: renomeie arroz para Arroz Tipo
            1 e troque o preço para 27,90.
          </TabsContent>
        </Tabs>
